...
from .detection import draw_bbox, get_recommended_box_thickness
...
def draw_skeleton(
    ...
    if edge_links is not None and edge_colors is not None and len(edge_links) != len(edge_colors):
        raise ValueError(
            "edge_colors and edge_links must have the same length")
    ...
    if len(keypoints.shape) != 2 or keypoints.shape[1] not in (2, 3):
        raise ValueError(
            f"Argument keypoints must be a 2D array of shape [Num Joints, 2] or [Num Joints, 3], got input of shape {keypoints.shape}")
    ...
    direction_from_center /= np.linalg.norm(
        direction_from_center, axis=1, ord=2, keepdims=True) + 1e-9
    ...
    for keypoint, score, direction, show, color in zip(keypoints, keypoint_scores, direction_from_center, keypoints_to_show_mask, keypoint_colors):
        ...
        if show_keypoint_confidence:
            ...
            y = int(cy + h // 2)
            cv2.putText(overlay, text, org=(x, y), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale=0.5, color=(250, 250, 250), thickness=1, lineType=cv2.LINE_AA)
    ...
    if show_confidence and len(confident_keypoints):
        ...
        overlay = draw_bbox(overlay, title=f"{score:.2f}", box_thickness=box_thickness, color=(
            255, 0, 255), x1=x, y1=y, x2=x + w, y2=y + h)
        ...
        *,
        ...
        for pose_index in range(num_poses):
            ...
            if boxes is not None:
                ...
                current_box_thickness = box_thickness or get_recommended_box_thickness(
                    x1, y1, x2, y2)
                current_joint_thickness = joint_thickness or current_box_thickness
                current_keypoint_radius = keypoint_radius or math.ceil(
                    current_box_thickness * 3 / 2)
            else:
